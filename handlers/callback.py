...
def handler_callback(bot: TeleBot, call: types.CallbackQuery):
    with Session(engine) as session:
        ...
        if call.data == "about_project":
            bot.send_photo(call.from_user.id, "https://i.postimg.cc/1zZF4vmQ/photo-2025-08-19-14-39-37.jpg", caption="""        
...
Мы делимся видео, статьями и отзывами, чтобы вы чувствовали себя частью нашего сообщества!
    """, reply_markup=markup)
        ...
        # ГОРОДА
            ...
            button3 = types.InlineKeyboardButton("Отзывы",callback_data="our_feedbacks") 
...
Для более подробной информации напишите основателю проекта: @irrrun
                             """)
...
<a href="https://yandex.ru/video/touch/preview/2593925414043493009">Слушать и смотреть клип ЗДЕСЬ</a>  
            ...
            bot.send_message(call.from_user.id, """
КАЛИНИНГРАД
Родина автора проекта 
За❤️Любовь Артема Николаева

Гостевой дом ВИШТЫНЕЦ!

♥️22 августа на двое суток
(с пятницы по воскресенье)
Всего 1 раз в этом году! 

Снимаем весь гостевой дом! 🔥
2 особняка!  На территории не будет чужих людей. 

🏠Огромная зеленая территория у озера. 
👩На территории 3 беседки, одна из которых большая - подходит для танцев. 
🍢Большой каминный зал. Кухня с обеденной зоной внутри и на улице под крышей. 

6 2х комнатных номеров с террасами (двуспальная кровать, +кровать), санузел, отдельная кухня.
🔥 Своя большая лужайка с мангалами и местом для отдыха. 
10 номеров с санузлами от 2х до 5 человек, из них только 2 номера для пары (только двуспальная кровать). 
7 номеров на Мансарде (эконом вариант). Туалет и душ на этаже.


Что нас ждёт:

☀️Купание в о.Виштынец на границе с Польшей
☀️Катание на Сапе
☀️Танцы Бачата и Сальса на свежем воздухе 
☀️Игра За💛Любовь от основателей
☀️Мастер-классы по танцам с нуля
☀️Шашлыки, костер, душевные посиделки
☀️Новые знакомства с потрясающими людьми

👩‍👧 Есть те, кто едет с детьми! 
Им есть, чем заняться: Футбольное поле. Баскетбол. Волейбол. 2 детские площадки, горка в озере. 

Заезд в номера в пятницу с 14.00. (Многие приезжают вечером после работы)
Освобождение номеров воскресенье в 12.00.
И можем еще оставаться на территории. Продолжаем танцевать, разучивать и записывать футворки. Готовить шашлык, купаться, гулять. 
...
В стоимость номеров включено: 
💛Проживание двое суток, 
💛Мастер-классы по бачате и сальсе, оупэн эйр со светомузыкой. 
💛Пользование каминным залом второго дома. 
💛Постельное белье и полотенца есть для всех! 
...
Остались вопросы?
Пиши 📱 8 916 675 82 99
                             """, parse_mode='html')
            
        elif call.data == "event_forlove":
            bot.send_message(call.from_user.id, """Для более подробной информации напишите: @irrrun""")
...
""", reply_markup=markup)
        ...
        # РЕФЕРАЛЬНАЯ СИСТЕМА
            ...
            button4 = types.InlineKeyboardButton("Подробнее о партнерке", callback_data="about_ref_program")
