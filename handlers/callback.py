...
Присоединяйтесь к движению, которое меняет жизни к лучшему!
    """)
            ...
            markup.add(button)
            bot.send_message(call.from_user.id, """        
Будьте в курсе всех новостей и событий проекта 'За любовь'! Подписывайтесь на наши социальные сети, чтобы следить за анонсами мероприятий, вдохновляющими историями участников и полезными материалами. Мы делимся видео, статьями и отзывами, чтобы вы чувствовали себя частью нашего сообщества!
    """, reply_markup=markup)
        ...
        # ГОРОДА
            ...
            bot.send_message(call.from_user.id, "Настольная игра 'За любовь' — это не просто развлечение, а уникальный способ познакомиться с новыми людьми, укрепить связи и обсудить важные жизненные темы в легкой и непринужденной атмосфере. Игра подходит для всех — от друзей до семейных пар. Запишитесь на ближайшее мероприятие в вашем городе и откройте для себя мир осознанного общения!", reply_markup=markup)
        elif call.data == "about_game":
            bot.send_message(call.from_user.id, """
Игра 'За любовь' создана для 1–12 участников и подходит для людей любого возраста, которые хотят лучше понять себя и других. В комплект входят карточки с глубокими вопросами, заданиями и сценариями, которые помогают раскрыться, обсудить ценности и выстроить доверие. Это идеальный способ начать свое путешествие в экосистеме 'За любовь'. Узнайте, как игра может изменить ваш взгляд на отношения!""")
            ...
            for city in cities:
                ...
                markup.add(button)
            bot.send_message(call.from_user.id, "Мы уже работаем в 12 городах России, и наша сеть растет! Выберите свой город, чтобы узнать о ближайших играх, познакомиться с организатором и присоединиться к местному сообществу 'За любовь'", reply_markup=markup)
...
Пользователь @{user.username} купил подписку {"на месяц"if pay_metadata.price == 333 else "на год"}
        ...
        #ПРОДУКТЫ
            ...
            bot.send_message(call.from_user.id, """
Хотите зарабатывать, делясь ценностями 'За любовь'? Наша бизнес-модель открывает множество возможностей: от проведения игр до управления городом или создания контента. Выберите роль, которая вам ближе, и начните свой путь к финансовой свободе и вдохновению. Мы поддержим вас на каждом шагу: предоставим обучение, маркетинговые материалы и доступ к нашей экосистеме!    
""",reply_markup=markup)
...
Пользователь @{user.username} купил продукт на {pay_metadata} рублей
        ...
        #МЕРОПРИЯТИЯ
            ...
            bot.send_message(call.from_user.id, """
Проект 'За любовь' — это не только игры, но и яркие оффлайн-мероприятия: фестивали знакомств, туры, конкурсы красоты и вдохновляющие встречи. Каждое событие создано, чтобы объединять людей, помогать находить друзей, партнеров или единомышленников. Узнайте больше о наших форматах, посмотрите расписание или станьте гидом, чтобы организовывать события в своем городе!
                             """, reply_markup=markup)
            ...
            bot.send_message(call.from_user.id, """
Мы проводим разные форматы мероприятий, чтобы каждый нашел что-то для себя. Выберите, что вас вдохновляет: фестивали, туры, конкурсы или мастер-классы.
Для более подробной информации напишите основателю проекта: @Forlove2025
                             """, reply_markup=markup)
...
{addiction_text}
            ...
            markup = types.InlineKeyboardMarkup()
            button = types.InlineKeyboardButton("Купить сертификат", url=pay_link)
            markup.add(button)
            
            bot.send_message(call.from_user.id, f"""
Информация:
{schedule.name}, {schedule.city}, {schedule.start}
!!! РЕАЛИЗОВАТЬ ОТПРАВКУ ПРИ ОПЛАТЕ 
""", reply_markup=markup)
            ...
            markup.add(button)
            bot.send_message(call.from_user.id, f"""
Мечтаете организовывать туры и мероприятия 'За любовь' в вашем городе? Станьте гидом и вдохновляйте людей на новые знакомства и развитие! Мы предоставим вам обучение, материалы и поддержку, чтобы вы могли создавать незабываемые события. Узнайте, как начать, и подайте заявку!
Для более подробной информации напишите основателю проекта: @Forlove2025

!!! РЕАЛИЗОВАТЬ ОТПРАВКУ ПРИ НАЖАТИИ НА КНОПКУ 
""", reply_markup=markup)
